...
func handleJoinRoom(d *sql.DB, h *hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := requireIdentity(d, w, r)
		if id == nil {
			return
...
func handleMove(d *sql.DB, h *hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := requireIdentity(d, w, r)
		if id == nil {
			return
...
func handleResign(d *sql.DB, h *hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := requireIdentity(d, w, r)
		if id == nil {
			return
...
func handleCancelRoom(d *sql.DB, h *hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := requireIdentity(d, w, r)
		if id == nil {
			return
...
func mountGameRooms(mux *http.ServeMux, d *sql.DB) {
	...
	mux.HandleFunc("POST "+base+"/join", httpx.RateLimit(20, handleJoinRoom(d, h)))
	mux.HandleFunc("GET "+base+"/{id}", handleGetRoom(d))
	mux.HandleFunc("DELETE "+base+"/{id}", handleCancelRoom(d, h))
	mux.HandleFunc("POST "+base+"/{id}/moves", handleMove(d, h))
	mux.HandleFunc("POST "+base+"/{id}/resign", handleResign(d, h))
